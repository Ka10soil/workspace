...
void arm_D(armmode_new_t mode) {
    ...
    switch (mode) {
        ...
        case UP:
            ...
            break;
        case SETNEW:
            if(now_arm_angle <= 60)ev3_motor_set_power(EV3_PORT_D, 20);
            else ev3_motor_set_power(EV3_PORT_D, -15);
            break;
        case SETNEW_2:
            if(now_arm_angle <= 40)ev3_motor_set_power(EV3_PORT_D, 20);
            else ev3_motor_set_power(EV3_PORT_D, -15);
            break;
        default:
            break;
    ...
    while (true) {
        now_arm_angle = ev3_motor_get_counts(EV3_PORT_D);
        if(now_arm_angle <= 51 && now_arm_angle >= 49 && mode == SETNEW) break;
        if(now_arm_angle <= 41 && now_arm_angle >= 39 && mode == SETNEW_2) break;
        if(mode == DOWN) break;
        if(mode == UP) break;
    }
    if(mode == SETNEW) {
        ...
        while (true) {
            now_arm_angle = ev3_motor_get_counts(EV3_PORT_D);
            if(now_arm_angle <= 78 && now_arm_angle >= 76 && mode == SETNEW) break;
        }
    }
