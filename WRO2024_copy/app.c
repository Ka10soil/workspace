...
void straight_off_no_reset(float cm, bool_t logic){
    int now_motor_angle_rc = 0;
    while (true) {
        now_motor_angle_rc = abs(ev3_motor_get_counts(EV3_PORT_B));
        if(cm*ROBOT1CM <= now_motor_angle_rc) break;
...
void area_2(){
    ...
    linetrace_cm_rgb_pd_SP(7, 20, true);
    arm_A(CLOSE);
    arm_D(THREE);
    linetrace_cm_rgb_pd_SP(8.5, 20, true);
...
void main_task(intptr_t unused) {
    ...
    if(start == 2){
        ...
        turn(90, -30, 30);
        speed_arm(EV3_PORT_D, DOWN, 40);
        linetrace_cm_rgb_pd_SP(5, 15, false);
        linetrace_rgb_pd_SP(RIGHT, COLOR_BLACK, 20, true);
        arm_D(DOWN);
        linetrace_cm_rgb_pd_SP(15, 20, true);
        turn(90, 30, -30);
        straight_on(-40);
        tslp_tsk(400*MSEC);
        arm_D(UP);
    ...
    turn(48, 30, -30);
    linetrace_cm_rgb_pd_SP(12, 20, false);
    linetrace_cm_rgb_pd_SP(8, 30, false);
    ...
    case 3:
        turn(180, 30, -30);
        linetrace_cm_rgb_pd_SP(20.6, 20, true);
        tslp_tsk(100*MSEC);
        turn(90, -30, 30);
    default:
        break;
    ...
    arm_D(THREE);
    turn(90, -30, 30);
    if(bg_color == 2) linetrace_cm_rgb_pd_SP(5, 20, false);
    if(bg_color == 3) {
