...
/*void turn(int angle, int lb_power, int rc_power){
    ...
    while (true) {
        ...
        if (lb_power == 0) {
            ...
            if (goal_angle - ((points + 5)*turn_num*ROBOT1CM) <= now_right_angle) changing_power = changing_power - 0.007;
            if (changing_power <= 15) changing_power = 15;
        ...
        if (rc_power == 0) {
            ...
            if (goal_angle - ((points + 5)*turn_num*ROBOT1CM) <= now_left_angle) changing_power = changing_power - 0.007;
...
void turn(int target_angle, int lb_power, int rc_power){
    ev3_motor_stop(EV3_PORT_B, true);
    ev3_motor_stop(EV3_PORT_C, true);
    tslp_tsk(100*MSEC);
    int lb_sign = lb_power / abs(lb_power);
    ...
    while (true)
    {
        ...
        d = now_rate;
        if(now_angle < target_angle / 3) p = now_angle + 30;
        ...
        if (now_angle > target_angle - 0.05){
...
void start_2() {
    ...
    linetrace_cm_rgb_pd_SP(60, 40, false);
...
void area_1(){
    ...
    linetrace_rgb_pd_SP(BOTH, COLOR_BLACK, 20, true);
    turn(180, 30, -30);
    linetrace_cm_rgb_pd_SP(13, 20, true);
    turn(40, 0, 30);
    arm_A(OPEN);
    arm_D(TWO);
    arm_A(CLOSE);
