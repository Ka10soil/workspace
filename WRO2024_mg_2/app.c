...
colorid_t mg_color[] = {0, 0, 0, 0, 0, 0};
...
void rg_check(port_t port, int num){
    ...
    if(green > red) mg_color[num] = COLOR_GREEN;
    else mg_color[num] = COLOR_RED;
...
void arm_A(armmode_new_t mode){
    ...
    switch (mode) {
        ...
        case CLOSE:
            ...
            break;
        case SET:
            if(now_arm_angle_A >= -260)ev3_motor_set_power(EV3_PORT_A, -20);
            else ev3_motor_set_power(EV3_PORT_A, 15);
            break;
        case GET_OBJ_2:
            if(now_arm_angle_A >= -275)ev3_motor_set_power(EV3_PORT_A, -20);
            else ev3_motor_set_power(EV3_PORT_A, 20);
            break;
        case GETDEBRIS:
    ...
    while (true) {
        now_arm_angle_A = ev3_motor_get_counts(EV3_PORT_A);
        if(now_arm_angle_A <= -259 && now_arm_angle_A >= -261 && mode == SET) break;
        if(now_arm_angle_A <= -274 && now_arm_angle_A >= -276 && mode == GET_OBJ_2) break;
